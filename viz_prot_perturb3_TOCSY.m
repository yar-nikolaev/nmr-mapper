...
select_dset = 2; % 0 IN115a measles; 1 PTBRRM1_SLH; 2 TOCSY
...
switch select_dset
    ...
    case 2 % TOCSY
        nmr_dataset_dirs = {
        '/Users/yar/Dropbox/Science/Projects/2015_NMR_peak_tracking/site_mapper/180316_FOXM1_23'
        };
        ...
        mapper_data_dirs = {
        '/Users/yar/Dropbox/Science/Projects/2015_NMR_peak_tracking/site_mapper/191017_FOXM1_TOCSY/analysis_23'
            };
        ...
        % can define one if same for all
        peaklist_paths = {
            '/Users/yar/Dropbox/Science/Projects/2015_NMR_peak_tracking/site_mapper/191017_FOXM1_TOCSY/peaklists/23.peaks'
            };    
        
        % Switching X-Axis data
        xaxis = 1; % 1 - spectrum id; 2 - time; 3 - 31P(RNA)
        flag_import_and_overlay_cara_manual_tracking = 1;
...
flag_drop_first_point = 0; % to count CSP from second spectrum. 

if flag_import_and_overlay_cara_manual_tracking    
%     cara_repo_path = '/Volumes/Data/yar/Dropbox/_eth2/project_Measles/NMR/spectra/190415_IN115a_pA20_co-P50N525_100uM_298K_600/IN115a_v2_trace.cara';
    cara_repo_path = '/Users/yar/Dropbox/Science/Projects/2015_NMR_peak_tracking/site_mapper/191017_FOXM1_TOCSY/191017_FOXM1_pklist_n_analysis.cara';    
%     target_project = 'v1';
%     target_peaklist = 'IN115a';
    target_project = 'xx';
    target_peaklist = '23';
%     optns.nmr_data_path = '/Volumes/Data/yar/Dropbox/_eth2/project_Measles/NMR/spectra/190415_IN115a_pA20_co-P50N525_100uM_298K_600';
    optns.nmr_data_path = '/Users/yar/Dropbox/Science/Projects/2015_NMR_peak_tracking/site_mapper/191017_FOXM1_TOCSY/180316_FOXM1_23';
    if flag_drop_first_point
        % This will drop first point BEFORE CSP calc, i.e. second spectrum
        % is counted as position ZERO.
        optns.drop_n_first_points = 1;
        fl = mapper.get_cara_peakshifts_04b_drop_some_points(cara_repo_path,target_project,target_peaklist,optns);
        traj_select{3} = fl.HN_csp;
        traj_select{4} = fl.amp;        
    else
        fl = mapper.get_cara_peakshifts_04(cara_repo_path,target_project,target_peaklist,optns);
...
%         fl.HN_csp = sqrt( ( fl.H_csp.^2+(fl.N_csp./5).^2 )./2 );
    ...
    % for traj_select{2}
    traj_select{4} = bsxfun(@rdivide, traj_select{4}, max(traj_select{4},[],1));
    traj_select{4} = traj_select{4} .* max(max(traj_select{1},[],2)); % max value of all CSPs.        
    
    % Adjust auxiliary options - to match 4 ploting sets
    optns.xaxis = repmat(optns.xaxis,2,1);
    optns.plotLW = repmat(optns.plotLW,2,1);
    optns.plotSym = repmat(optns.plotSym,2,1);
    optns.legend = [optns.legend; 'manual CSP'; 'manual int'];
end;
...
if flag_mapper_vs_cara_select
    ...
    traj_select(drop_sets) = [];
    optns.xaxis(drop_sets) = [];
    optns.plotLW(drop_sets) = [];
