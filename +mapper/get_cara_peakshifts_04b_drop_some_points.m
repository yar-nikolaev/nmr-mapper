function full_peaklist = get_cara_peakshifts_04b_drop_some_points(cara_repo_path, target_project, target_peaklist, optns)
...
if isfield(optns, 'drop_n_first_points') && ~isempty(optns.drop_n_first_points)    
    drop_pts = optns.drop_n_first_points;
    fl.H(:,drop_pts) = [];
    fl.N(:,drop_pts) = [];
    fl.amp(:,drop_pts) = [];
    fl.spec_ids(drop_pts) = [];
    fl.spec_ids2(drop_pts) = [];
    fl.time = 1:numel(fl.spec_ids);    
end
